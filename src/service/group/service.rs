...
impl GroupService {
    pub fn new(
        ...
        Self { conn, group_repository, empty_profile }
    }
    ...
    { 
        ...
        let res = GroupRepository::create_group_with_executor(
            conn,
            group_name
        ).await;
        let group = match res {
        ...
        for mem in members {
            let conn = tx.acquire().await?;
            let res = GroupRepository
                ::add_user_to_group_with_executor(
                    conn, 
                    mem, 
                    group_id
                )
        ...
        let conn = tx.acquire().await?;
        let res = GroupRepository
            ::set_profile_image_for_group_with_executor(
                conn, 
                group.id, 
