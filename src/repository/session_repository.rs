use sea_orm::{DatabaseConnection, ActiveValue, ActiveModelTrait, DbErr, EntityTrait, QueryFilter, ColumnTrait};
...
impl SessionRepository {
    ...
    pub async fn find_sessions_by_user_id(
        ...
        let query = session::Entity::find().filter(session::Column::UserId.eq(user_id)).all(&self.conn);
        query.await
    }
}